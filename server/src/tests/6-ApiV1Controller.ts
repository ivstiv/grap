...
describe("/api/v1/address", () => {
  ...
  it("Should return email address with session", async () => {
    ...
    assert.strictEqual(userAddressesBefore, 1); // start from 1 because of the previous test
    assert.strictEqual(userAddressesAfter, 2);
  });
});



describe("/api/v1/inbox/:address/latest", () => {
